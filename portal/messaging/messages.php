        ...
        (function () {
            ...
            app.controller('inboxCtrl', ['$scope', '$filter', '$http', '$window', '$q', function ($scope, $filter, $http, $window, $q) {
                ...
                $scope.readMessage = function (idx) {
                    // Check if items[idx] exists and has message_status property
                    if (!$scope.items || !$scope.items[idx] || !$scope.items[idx].hasOwnProperty('message_status')) {
                        return; // Exit early if item doesn't exist or doesn't have message_status
                    }
                    ...
                    $scope.isTrash = $scope.isSent = $scope.isInbox = false;
                    $scope.items = $scope.allItems;
                    if ($scope.items && $scope.items[idx]) {
                            ...
                            <table class="table table-striped table-bordered table-hover refresh-container pull-down">
                                ...
                                <tr ng-repeat="item in pagedItems[currentPage]" role='button'>
                                    ...
                                    <td role = "button" class="message-row">
                                        <span class="col-sm-1" style="max-width: 5px;"><input type="checkbox" checklist-model="item.deleted" value={{item.deleted}} ng-click="$event.stopPropagation()"></span>
