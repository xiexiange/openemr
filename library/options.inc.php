    ...
    while ($prow = sqlFetchArray($pres)) {
        ...
        while ($frow = sqlFetchArray($fres)) {
            ...
            while ($group_fields = sqlFetchArray($group_fields_query)) {
                ...
                // For new patients, force allow_patient_portal to YES and disable the field
                global $pid;
                if (empty($pid) && $field_id == 'allow_patient_portal') {
                    $currvalue = 'YES';
                    // Disable the field by adding '0' to edit_options (0 = disabled)
                    if (empty($group_fields['edit_options'])) {
                        $group_fields['edit_options'] = '0';
                    } elseif (strpos((string) $group_fields['edit_options'], '0') === false) {
                        $group_fields['edit_options'] .= '0';
                    }
                }

                // For non-admin users creating new patients, force providerID to current user and disable the field
                if (empty($pid) && $field_id == 'providerID') {
                    require_once(__DIR__ . "/../Common/Acl/AclMain.php");
                    if (!\OpenEMR\Common\Acl\AclMain::aclCheckCore('admin', 'super')) {
                        $currentUserId = $_SESSION['authUserID'] ?? null;
                        if ($currentUserId) {
                            $currvalue = $currentUserId;
                            // Disable the field by adding '0' to edit_options (0 = disabled)
                            if (empty($group_fields['edit_options'])) {
                                $group_fields['edit_options'] = '0';
                            } elseif (strpos((string) $group_fields['edit_options'], '0') === false) {
