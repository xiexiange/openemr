...
if (isset($_GET['sSearch']) && $_GET['sSearch'] !== "") {
    ...
    // Prevent empty string after trim from generating %% pattern that matches all records
    // If empty, skip building search conditions so only customWhere (with providerID restriction) is used
    if ($sSearch !== "") {
        foreach ($aColumns as $colname) {
            ...
            if ($colname == 'name') {
                ...
                } elseif ($searchAny) { // any search - use wildcards on both sides
                    array_push($srch_bind, ('%' . $sSearch . '%'), ('%' . $sSearch . '%'), ('%' . $sSearch . '%'));
                } else {// like search - prefix only
                    array_push($srch_bind, ($sSearch . "%"), ($sSearch . "%"), ($sSearch . "%"));
                }
...
for ($i = 0; $i < count($aColumns); ++$i) {
    ...
    if (isset($_GET["bSearchable_$i"]) && $_GET["bSearchable_$i"] == "true" && $_GET["sSearch_$i"] != '') {
        $sSearch = trim((string) $_GET["sSearch_$i"]);
        // Prevent empty string after trim from generating %% pattern that matches all records
        if ($sSearch === "") {
            continue;
        }
...
$srch_bind = array_merge($customBindValues, $srch_bind);
...
$totalBindValues = $boundFilter->getBoundValues();
$row = sqlQuery("SELECT COUNT(id) AS count FROM patient_data WHERE $customWhere", $totalBindValues);
$iTotal = $row['count'];
