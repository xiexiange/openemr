...
function menuActionClick(data,evt)
{
    ...
    if(data.enabled())
    {
        ...
        navigateTab(webroot_url + dataurl, data.target, function () {
            ...
            // H5：选中了某个菜单项（包括二级/三级）并成功切换 tab 后，
            // 通过触发导航栏的 hamburger 按钮点击来“收起整个菜单列表”
            // 延迟收起菜单，确保在所有其他代码（包括 iframe 加载完成后的处理）执行完成后再收起
...
function clearPatient(openFinder = true)
{
    ...
    tabCloseByName('pat');
    if (openFinder) {
        navigateTab(webroot_url+'/interface/mobile/main/finder/dynamic_finder.php','fin', function () {
...
function openExistingTab(url, name) {
    for (let tabIdx = 0; tabIdx < app_view_model.application_data.tabs.tabsList().length; tabIdx++) {
        ...
        switch (currTabUrl) {
            ...
            case webroot_url + '/interface/main/finder/dynamic_finder.php':
                currTabUrl = webroot_url + '/interface/mobile/main/finder/dynamic_finder.php';
                break;
            ...
            case '../../../interface/main/messages/messages.php?form_active=1':
                currTabUrl = webroot_url + '/interface/mobile/main/messages/messages.php?form_active=1';
                break;
        }
        // Also check if the URL contains the old desktop path and normalize it
        if (currTabUrl && currTabUrl.indexOf('/interface/main/finder/dynamic_finder.php') !== -1 && 
            currTabUrl.indexOf('/interface/mobile/') === -1) {
            currTabUrl = webroot_url + '/interface/mobile/main/finder/dynamic_finder.php';
            // Update the tab URL if it was the old path
            if (currTab.url().indexOf('/interface/main/finder/dynamic_finder.php') !== -1) {
                currTab.url(currTabUrl);
            }
        }
        if (url === currTabUrl) {
