...
<form method='post' name='theform' action='add_edit_drug.php?drug=<?php echo attr_url($drug_id); ?>'
 onsubmit='return validate(this);'>
    ...
    <div class="form-group mt-3">
        ...
        <!-- NOTE: onclick event removed to prevent RXCUI code lookup error -->
        <!-- Original onclick: sel_related("?codetype=RXCUI&limit=1&target_element=form_drug_code") -->
        <!-- To restore: uncomment the onclick attribute below -->
        <input class="form-control w-100" type="text" size="50" name="form_drug_code" value='<?php echo attr($row['drug_code']) ?>'
             title='<?php echo xla('RXCUI code (click event disabled)'); ?>' data-toggle="tooltip" data-placement="top" readonly />
    </div>
