...
// Need to go up two levels from interface/mobile to reach openemr root
$checkerPath = realpath(dirname(__DIR__, 2) . "/src/Common/Compatibility/Checker.php");
if ($checkerPath === false) {
    // Fallback: try from __DIR__ directly
    $checkerPath = realpath(__DIR__ . "/../../src/Common/Compatibility/Checker.php");
}
if ($checkerPath !== false) {
    ...
    // If checker file not found, log error but don't die (for development)
    error_log("Warning: Compatibility Checker.php not found. Tried: " . dirname(__DIR__, 2) . "/src/Common/Compatibility/Checker.php");
}
...
// From interface/mobile/globals.php, need to go up 3 levels to reach openemr root
// (globals.php -> mobile -> interface -> openemr)
$webserver_root = dirname(__FILE__, 3);
if (IS_WINDOWS) {
...
// From interface/mobile, need to go up 2 levels to reach openemr root, then library
require_once(dirname(__DIR__, 2) . "/library/sql.inc.php");
...
// From interface/mobile, need to go up 2 levels to reach openemr root
