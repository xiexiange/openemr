...
$csrfToken = $_POST["csrf_token_form"] ?? null;
if (empty($csrfToken)) {
    error_log("OpenEMR Error: CSRF token is missing in POST data for new_comprehensive_save.php");
    CsrfUtils::csrfNotVerified();
}
if (!CsrfUtils::verifyCsrfToken($csrfToken)) {
    error_log("OpenEMR Error: CSRF token verification failed for new_comprehensive_save.php. Token: " . substr($csrfToken, 0, 10) . "... Session key exists: " . (isset($_SESSION['csrf_private_key']) ? 'yes' : 'no'));
    CsrfUtils::csrfNotVerified();
}
...
// Only do this if session is valid and user is authenticated
if (isset($_SESSION['authUserID']) && !AclMain::aclCheckCore('admin', 'super')) {
